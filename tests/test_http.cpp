...
BOOST_AUTO_TEST_CASE(test_parse_get_request) {
    ...
    BOOST_CHECK_EQUAL(httpRequest.GetMethodString(), "GET");
...
BOOST_AUTO_TEST_CASE(test_parse_post_request_with_body) {
    ...
    BOOST_CHECK(httpRequest.Parse(buffer));
    BOOST_CHECK(httpRequest.IsFinished());
    BOOST_CHECK_EQUAL(httpRequest.GetMethodString(), "POST");
...
BOOST_AUTO_TEST_CASE(test_error_response) {
    auto response =
        bre::HttpResponse::MakeErrorResponse(bre::HttpStatus::NOT_FOUND, "Page not found");
...
struct FileTestFixture {
    ...
    ~FileTestFixture() { std::filesystem::remove(testFile); }
};
